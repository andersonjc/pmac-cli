 ...
 */
export function parseBacklogYaml(yamlContent: string, options: ParseOptions = {}): ParseResult<ProjectBacklog> {
 ...
 */
export async function parseBacklogFile(filePath: string, _options: ParseOptions = {}): Promise<ParseResult<ProjectBacklog>> {
  try {
    ...
    return {
      success: false,
      error: 'File system access not available in browser environment. Use parseBacklogYaml() with YAML content instead.'
    };
 ...
 */
export function validateBacklogSchema(data: any, options: { strict?: boolean; allowPartial?: boolean } = {}): ParseResult<void> {
...
function validateTask(task: any, fieldPath: string, _strict: boolean): ValidationError[] {
  ...
  const requiredFields = ['id', 'title', 'status', 'priority', 'estimated_hours', 'requirements', 'dependencies', 'blocks', 'notes'];
  for (const field of requiredFields) {
  ...
  if (task.estimated_hours && typeof task.estimated_hours !== 'number') {
    errors.push({ field: `${fieldPath}.estimated_hours`, message: 'Estimated hours must be a number' });
  }
...
function validateEpicSummary(epicSummary: any, _strict: boolean): ValidationError[] {
  ...
  if (epicSummary.total_estimated_hours && typeof epicSummary.total_estimated_hours !== 'number') {
    errors.push({ field: 'epic_summary.total_estimated_hours', message: 'Total estimated hours must be a number' });
  }
  ...
  for (const [level, riskList] of Object.entries(risks)) {
    ...
    for (let i = 0; i < riskList.length; i++) {
      ...
      if (!risk.risk || typeof risk.risk !== 'string') {
        errors.push({ field: `risks.${level}[${i}].risk`, message: 'Risk description must be a string' });
      }
      
      if (!risk.mitigation || typeof risk.mitigation !== 'string') {
        errors.push({ field: `risks.${level}[${i}].mitigation`, message: 'Risk mitigation must be a string' });
      }
    }
  }
 ...
 */
function calculateLongestPath(nodeId: string, nodeMap: Map<string, DependencyNode>, visited: Set<string>): number {
