<script lang="ts">
  ...
  $: progressPercentage = task.actual_hours && task.estimated_hours 
    ? Math.min((task.actual_hours / task.estimated_hours) * 100, 100)
    : task.status === 'completed' ? 100 : 0;
    
  // Format status text for display
  $: statusText = task.status === 'in_progress' ? 'in progress' 
  ...
  <header class="flex items-start justify-between mb-3">
    <div class="flex-1 min-w-0">
      <div class="flex items-center gap-2 mb-1">
        ...
        <h3 class="text-gray-100 font-medium truncate">{task.title}</h3>
      </div>
